...
*****This script however, is intended to change the basis vectors of the CMD to allow the selection of specific regions of it. I.E. draw a box along the length of the WDs with one axis parallel-ish to the cooling tracks.
...
output_name= 'test_subsetlots.csv'
...
mg_string= "(phot_g_mean_mag+5*log10(parallax+(" + str(parallax_correction)+"))-10)"

search_statement = "SELECT source_id, ra, dec,  " +  mg_string + " AS mg, bp_rp, " + " -1*" +str(basis_coeff1) + "* bp_rp +" + str(basis_coeff2) + "* " + mg_string + " AS xprime, " + str(basis_coeff2) + "*bp_rp + " + str(basis_coeff1) + "* " + mg_string + " AS yprime FROM"

search_statement= search_statement+ " gaiadr2.gaia_source WHERE parallax_over_error > 10 AND phot_g_mean_flux_over_error>50 AND phot_rp_mean_flux_over_error>20 AND phot_bp_mean_flux_over_error>20 AND phot_bp_rp_excess_factor < 1.3+0.06*power(phot_bp_mean_mag-phot_rp_mean_mag,2) AND phot_bp_rp_excess_factor > 1.0+0.015*power(phot_bp_mean_mag-phot_rp_mean_mag,2) AND visibility_periods_used>8 AND astrometric_chi2_al/(astrometric_n_good_obs_al-5)<1.44*greatest(1,exp(-0.4*(phot_g_mean_mag-19.5)))" #modified to have number limit search statement from Gaia DR2HRD 2018
#search_statement= search_statement + " AND parallax > " + str(parallax_min)
...
def asynchronous_query():
    ...
    #print(search_statement)
    job = Gaia.launch_job_async(search_statement, dump_to_file=False)
    r= job.get_results()
...
output_table.write(output_name, format = 'ascii.tab', overwrite= True)
...
print("Logging out.")
Gaia.logout()
print("Log out successful (presumably).")
