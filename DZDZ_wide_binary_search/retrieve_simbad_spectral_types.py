"""
Created by Ben Kaiser (UNC-Chapel Hill) 2025-07-31

Created with the help of Copilot

Updated to run on the white dwarf+white dwarf wide binaries.
...
#input_name='WDWD_widebinaries_eDR3_highconf.csv'
#input_name='WDWD_widebinaries_eDR3_highconf_simbadadded1.csv'
#input_name='WDWD_widebinaries_eDR3.csv'
input_name='WDWD_widebinaries_eDR3_simbadadded1.csv'
...
def query_simbad_name_and_sptype(source_ids, chunk_size=50):
    """
    ...
    simbad.add_votable_fields('ids', 'main_id', 'sp_type','sp_qual')
    ...
    for start in range(0, len(source_ids), chunk_size):
        ...
        tables.append(
            Table([sids, simbad_names, sptypes,spquals],
                  names=('source_id', 'simbad_name', 'simbad_sp_type','simbad_sp_qual'),dtype=('i8', 'U50', 'U50','U50'))
        )
...
def query_simbad_all_gaia_ids(source_ids, chunk_size=50):
    """
...
simbad_matches=query_simbad_name_and_sptype(input_table['source_id'+num])
for name in simbad_matches.colnames:
    simbad_matches.rename_column(name,name+num)
#simbad_matches=query_simbad_all_gaia_ids(input_table['GaiaEDR3'])
simbad_matches['source_id'+num].pprint()
print(len(simbad_matches),len(input_table))
merged=join(input_table,simbad_matches, keys_left='source_id'+num,keys_right='source_id'+num, join_type='left')
merged.remove_column('source_id'+num+'_2')
...
output_name=input_name.split('.')[0]+'_simbadadded'+num+'.csv'
