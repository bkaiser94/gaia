...
#input_file='WDWD_widebinaries_eDR3_highconf.csv'
#input_file='WDWD_widebinaries_eDR3_highconf_Vincent2023bClass.csv'
input_file='WDWD_widebinaries_eDR3_simbadadded1_simbadadded2_DZcands_Vincent2023bClass1.csv'


num="2"
output_file=input_file.split('.')[0]+'_Vincent2023bClass'+num+'.csv'
...
#rename colnames to have the num

for name in vincent_table.colnames:
    vincent_table.rename_column(name,name+num)

#joined_table=join(input_table,vincent_table,keys='source_id',join_type='left')
joined_table=join(input_table,vincent_table,keys_left='source_id'+num,keys_right='source_id'+num,join_type='left')
joined_table.pprint()

joined_table.write(output_file)
sys.exit()
