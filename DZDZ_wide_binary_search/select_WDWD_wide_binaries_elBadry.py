...
elBadry_file='all_columns_catalog.fits.gz'
#elBadry_file='all_columns_catalog_shift.fits'

elBadry_dir='/Users/BenKaiser/Desktop/elBadry_catalogue/'
output_dir='/Users/BenKaiser/Desktop/gaia/DZDZ_wide_binary_search/'
#output_filename='dimWDMS_eDR3.fits'
#output_filename='dimWDMS_eDR3_radec.csv'
#output_filename='dimWDMS_F0toK7_eDR3_highconf.fits'
#output_filename='WDWD_widebinaries_eDR3_highconf.csv'
output_filename='WDWD_widebinaries_eDR3.csv'
...
unique_inds=np.unique(elBadry_full_table['binary_type'])
print(unique_inds)
#print(elBadry_full_table['binary_type'][unique_inds])
wdwd_indices=np.where(elBadry_full_table['binary_type']=='WDWD')


wdwd_table=elBadry_full_table[wdwd_indices]
#wdwd_table=elBadry_full_table[0:100]
#wdms_table.pprint()
print(len(wdwd_table))

def get_abs_mag(band='g',comp=1):
    #abs_mag=wdms_table['phot_'+band+'_mean_mag'+str(comp)]+5*np.log10(wdms_table['parallax'+str(comp)])-10
    abs_mag=wdwd_table['phot_'+band+'_mean_mag'+str(comp)]+5*np.log10(wdwd_table['parallax'+'1'])-10 #El Badry uses the parallax of the primary to get the absolute magnitude of the secondary because it usually has better precision I'm pretty sure...
    return abs_mag

abs_g_mag1=get_abs_mag(comp=1)
abs_g_mag2=get_abs_mag(comp=2)

wdwd_table.add_column(abs_g_mag1,name='abs_g_mag1')
wdwd_table.add_column(abs_g_mag2,name='abs_g_mag2')
...
#highconf=np.where(wdwd_table['R_chance_align']<chance_align_cut)
#wdwd_table=wdwd_table[highconf]
#print('\n\n\nR<',chance_align_cut)
wdwd_table.pprint()

wdwd_table.write(output_filename,format='ascii.csv')
