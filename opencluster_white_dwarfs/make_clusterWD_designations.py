"""
Make designations for the white dwarfs in each cluster, so I can keep track of them since there is more than one per cluster, but I don't want to just give them coordinate WD J names because then it will take more effort to know which cluster each one belongs to for cross-referencing.
...
input_name='HR24members_GF21maincat_crossmatch_simbadadded_mwddadded_wdagesadded_agediffsig_gaiaadded.csv'
output_name=input_name.split('.')[0]+'_kaisernames.csv'
...
gf_sorted_inds=np.argsort(gf['ra'])
gf_sorted=gf[gf_sorted_inds]
ids=np.unique(gf["Name"])
print('n_ids:',ids.shape)

desig_col=Column(np.full(len(gf_sorted),'11111111111111111111111111111'),name='kcname')
def make_designation(name_col):
    numbers=np.int_(np.arange(1,len(name_col)+1,1))
    desig_arr=np.full(len(name_col),'_kcWD')
    merged_list=[]
    for first,second in zip(desig_arr,numbers):
        print('first',first,'second',second)
        merged_list.append(first+str(second))
    print('merged_list',merged_list)
    #desig_arr=desig_arr+numbers
    #print(desig_arr)
    return merged_list

for name in ids:
    name_inds=np.where(gf_sorted['Name']==name)
    name_col=gf_sorted[name_inds]['Name']
    desig_arr=make_designation(name_col)
    desig_col[name_inds]=desig_arr
print(desig_col)
...
desig_col=gf_sorted['Name']+desig_col
print(desig_col)
new_table=hstack([gf_sorted,desig_col])
new_table.rename_column('Name_2','kcname')
