...
spt.start_ApJ_fig(width_cols=2,width_height=[1.,2./(1+np.sqrt(5.))])
fig, ax = plt.subplots()
def plot_gaia_passbands(plot_all=False, render_main_plot=True):
    ...
    def plot_tuple(intuple, label, color, linestyle= gaia_linestyle):
    ...
    plt.text(4000,0.55, r'$G_{\mathrm{BP}}$', ha='right' ,color='b')
    plt.text(5500,0.75, r'$G$', ha='center' ,color='g')
    plt.text(6600,0.8, r'$G_{\mathrm{RP}}$', ha='right' ,color='r')

    #plt.text(4800,ps_height, r'$g$', ha='center' )
    #plt.text(4800,ps_height, r'$g$', ha='center' )
...
def plot_panstarrs_passbands(render_main_plot=True):
    ...
    plt.plot(ps1_wavelengths, ps1_all[2], label= 'PS1-gp1', color='k', linestyle=ps1_linestyle)
    ax.fill_between(ps1_wavelengths, ps1_all[2], 0, facecolor='g', alpha=0.2)
    
    plt.plot(ps1_wavelengths, ps1_all[3], label='PS1-rp1', color='k', linestyle=ps1_linestyle)
    ax.fill_between(ps1_wavelengths, ps1_all[3], 0, facecolor='r', alpha=0.2)
    plt.plot(ps1_wavelengths, ps1_all[4] , label='PS1-ip1', color='k', linestyle=ps1_linestyle)
    ax.fill_between(ps1_wavelengths, ps1_all[4], 0, facecolor='#871812', alpha=0.2)
    plt.plot(ps1_wavelengths, ps1_all[5], label='PS1-zp1', color='k', linestyle=ps1_linestyle)
    ax.fill_between(ps1_wavelengths, ps1_all[5], 0, facecolor='grey', alpha=0.2)
    plt.plot(ps1_wavelengths, ps1_all[6], label='PS1-yp1', color='k', linestyle=ps1_linestyle)
    ...
    #plt.legend(loc='best')
    plt.text(4800,ps_height, r'$g$', ha='center' )
    plt.text(6100,ps_height, r'$r$', ha='center' )
    plt.text(7500,ps_height, r'$i$', ha='center' )
    plt.text(8700,ps_height, r'$z$', ha='center' )
    plt.text(9700,ps_height, r'$Y$', ha='center' )
...
if __name__ == '__main__':
    #spt.initiate_science_plot()
    #spt.start_ApJ_fig(width_cols=2,width_height=[1.,2./(1+np.sqrt(5.))])
    plot_panstarrs_passbands(render_main_plot=False)
    plot_gaia_passbands(plot_all=False, render_main_plot=False)
    output_name='Gaia_DR2_passbands_wPanSTARRS_grizY_'+spt.time_string()+'.pdf'
