...
def plot_target_table(input_table, absmag='g', colours= ['bp', 'rp']):
    for row in input_table:
        ...
        plt.errorbar(np.copy(target_colour_dif), np.copy(target_absmag), yerr = np.copy(target_absmag_err),  xerr = np.copy(target_colour_dif_err), marker = 'o', markersize = 8, color = list_color, capsize = 4, label = target_label, linestyle ='none')
        print(target_colour_dif, target_absmag)
        print(row['name'])
        
        plt.annotate(str(row['name']),xy=(np.copy(target_colour_dif), np.copy(target_absmag)), xycoords='data', xytext=(np.copy(target_colour_dif+0.1),np.copy(target_absmag-0.1)), textcoords= 'data' , fontsize=8, color =list_color)
    return
